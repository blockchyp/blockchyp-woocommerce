 ...
 * Plugin Name
 *
 * @package   blockchyp-woocommerce
 * @author    BlockChyp, Inc.
 * @copyright 2020 BlockChyp
 *
 * @wordpress-plugin
		...
		public function payment_fields()
		{
			...
			<script>
				...
				jQuery(document).ready(function() {
					...
					<?php
						echo "tokenizer.gatewayHost = '{$this->gateway_host}'\n\t\t\t\t\t";
						echo "tokenizer.testGatewayHost = '{$this->test_gateway_host}';\n\t\t\t\t\t";
						echo "tokenizer.render('{$this->tokenizing_key}', {$testmode}, 'secure-input', options);\n";
					?>
				});
				jQuery('form.woocommerce-checkout').on('checkout_place_order', function (e) {
					...
					if (!blockchyp_enrolled) {
						...
						if (!tokenInput) {
							...
							<?php
								echo "tokenizer.tokenize('{$this->tokenizing_key}', req)\n";
							?>
