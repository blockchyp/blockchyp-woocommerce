...
/**
 * Initialize the BlockChyp Woocommerce plugin
 *
 * @throws Exception Exception The exception descriptor.
 */
function blockchyp_woocommerce_init() {
	...
	class WC_BlockChyp extends WC_Payment_Gateway {
		...
		public function payment_fields() {
			...
			<script>
				...
				jQuery(document).ready(function() {
					...
					tokenizer.gatewayHost = '<?php $this->gateway_host; ?>';
					tokenizer.testGatewayHost = '<?php $this->test_gateway_host; ?>';
					tokenizer.render('<?php $this->tokenizing_key; ?>', <?php $testmode; ?>, 'secure-input', options);
				});
				jQuery('form.woocommerce-checkout').on('checkout_place_order', function (e) {
					...
					if (!blockchyp_enrolled) {
						...
						if (!tokenInput) {
							...
							tokenizer.tokenize('<?php $this->tokenizing_key; ?>', req)
			...
			<?php
			if ( 'yes' === $this->settings['render_postalcode'] ) {
				?>
				<div>
		 ...
		 * @throws Exception $responseDescription The response description.
		 * @return array
		 **/
		public function process_payment( $order_id ) {
			...
			try {
				...
				if ( 'no_match' === $response['avsResponse'] ) {
					$order->add_order_note(
						sprintf(
